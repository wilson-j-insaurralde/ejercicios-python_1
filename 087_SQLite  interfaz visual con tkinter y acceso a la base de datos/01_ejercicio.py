...
class aplicacion:
    def __init__(self):
        self.articulos1=articulos.articulos()
        self.ventana1=tk.Tk()
    ...
    def carga_de_articulos(self):
        ...
        self.entry1=ttk.Entry(self.labelframe1,width=20,textvariable=self.descripcion_ingreso)
        self.entry1.grid(column=1,row=0,padx=4,pady=4)
        self.label2=ttk.Label(self.labelframe1,text="precio")
        self.label2.grid(column=0,row=1,padx=4,pady=4)
        self.precio_ingreso=tk.StringVar()
        self.entry2=ttk.Entry(self.labelframe1,width=20,textvariable=self.precio_ingreso)
        self.entry2.grid(column=1,row=1,padx=4,pady=4)
        self.boton1=ttk.Button(self.labelframe1,text="confirmar",command=self.articulo_bt_cargar)
        self.boton1.grid(column=1,row=2,padx=4,pady=4)
    def articulo_bt_cargar(self):
        datos=(self.descripcion_ingreso.get(),self.precio_ingreso.get())
        repuesta=self.articulos1.alta(datos)
        mb.showinfo("informacion","los datos fueron cargados correctamente")
        self.descripcion_ingreso.set("")
        self.precio_ingreso.set("")
    ...
    def consulta_por_codigo(self):
        ...
        self.boton2=ttk.Button(self.labelframe2,text="consulta",command=self.consulta)
        self.boton2.grid(column=1,row=3,padx=4,pady=4)
    def consulta(self):
        codigo=(self.codigo_consulta.get(),)
        repuesta=self.articulos1.consultar(codigo)
        if len(repuesta)>0:
    ...
    def listado_completo(self):
        ...
        self.boton3=ttk.Button(self.labelframe3,text="consultar",command=self.listar)
    ...
    def listar(self):
        repuesta=self.articulos1.recuperar_todos()
        self.scrolledtext1.delete("1.0", tk.END)
        for fila in repuesta:
